...
module('Integration | Component | public-dom-with-mixin', function(hooks) {
  setupRenderingTest(hooks);

  test('it exposes methods on the dom', async function(assert){
    await render(hbs`{{public-dom-with-mixin}}`);
  ...
  test('it can receive values', async function(assert){
    await render(hbs`{{public-dom-with-mixin value=100}}`);

    // get our element in the dom
    let element = this.element.firstChild;

    assert.equal( element.getValue(), 100, "supplied value is 100" );
    assert.equal( element.getValueString(), "Value is 100", "Default computed property value" );
